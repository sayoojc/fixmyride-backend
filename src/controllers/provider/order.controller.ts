...
import { GetOrderResponseDTO,ErrorResponseDTO,GetOrderResponseSchema } from "../../dtos/controllers/provider/providerOrder.controller.dto";
  ...
  async getOrderData(
    ...
    try {
      ...
      if(!order){
        ...
        res.status(StatusCode.NOT_FOUND).json({success:false,message:RESPONSE_MESSAGES.RESOURCE_NOT_FOUND('order')});
        return ;
      }
      const response = {
          success: true,
        ...
        if(!validate){
            ...
            res.status(StatusCode.INTERNAL_SERVER_ERROR).json({success:false,message:RESPONSE_MESSAGES.INTERNAL_SERVER_ERROR});
            return;
        }
      res
